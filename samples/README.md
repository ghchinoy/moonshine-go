...
## Native / In-Process Embedding (`pkg/moonshine`)

For Go applications that do not run a `moonshine serve` daemon or network IPC connection, [`pkg/moonshine`](../pkg/moonshine) provides a public, pure-Go (`CGO_ENABLED=0`) C-API reference binding over `libmoonshine`. For production bundling guidance (macOS `.app`, Windows, Linux, and MCP servers), see [`docs/bundling-libmoonshine.md`](../docs/bundling-libmoonshine.md).
