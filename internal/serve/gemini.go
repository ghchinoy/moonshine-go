...
// ToolResponse represents the execution result of a tool call returned to the LLM.
...
// Turn represents a single message turn in the multi-turn agent conversation history.
type Turn struct {
...
// LLMClient abstracts communication with the LLM model (e.g. real Gemini vs fake test client).
...
func (g *GeminiAgent) OnFinalizedLine(ctx context.Context, line moonshine.Line) []event.ActionRequest {
